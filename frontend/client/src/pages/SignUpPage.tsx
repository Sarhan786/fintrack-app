      ...
      <Box sx={{ marginTop: 8, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        {step === 2 && (
           <Box component="form" onSubmit={handleConfirmSignUp} sx={{ mt: 3 }}>
            <Typography sx={{textAlign: 'center', mb: 2}}>A confirmation code has been sent to {email}.</Typography>
            <TextField margin="normal" required fullWidth id="confirmationCode" label="Confirmation Code" name="confirmationCode" autoFocus value={confirmationCode} onChange={e => setConfirmationCode(e.target.value)} />
            {error && <Alert severity="error" sx={{ mt: 2 }}>{error}</Alert>}
            <Button type="submit" fullWidth variant="contained" sx={{ mt: 3, mb: 2 }} disabled={loading}>
